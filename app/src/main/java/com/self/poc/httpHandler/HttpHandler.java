...
/**
 * This class is used for handling Server REST API calls.
 * <p/>
 * The supported response types (ResponseBodyType) include {@link JSONArray}, {@link JSONObject} and {@link String}.
 * The supported request requestBodyType types (RequestBodyBean) include {@link JSONArray} and {@link JSONObject}.
 * The supported response bean types (ResponseBeanType) include all classes (not interface) that has public default constructor
 * and corresponding getters and setters that match the response json keys. In case keys do not match,
 * the data will not be present in the returned object. as well it can be null.
 * <p/>
 * In case the response is JSONObject of a List of bean class and contains only key that corresponds to the List,
 * you can still send the response bean type as ArrayList of Bean.
 * <p/>
 * Created by prakash on 3/9/15.
 */
